...
<body>
	...
	<form id="fbean" name="fbean" method="post"
		action="http://localhost:8080/myapp/save">

		<h1>${画面タイトル}</h1>
		<div>
			<label>番号</label> <input type="text" value="${番号}" name="番号" ${モード == '編集' ? 'disabled' : ''}/>
		</div>
		<br>
		<div>
			<label>姓名</label> <input type="text" value="${姓名}" name="姓名" />
		</div>
		<br>
		<div>
		...
		<br>
		<div>
			<label>生年月日</label> <input type="text" value="${生年月日}"
				placeholder="YYYY/MM/DD" type="text" name="生年月日" />

		</div>
		<br>
		<div>
			<label>入社年月日</label> <input type="text" Value="${入社年月日}" name="入社年月日"
				placeholder="YYYY/MM/DD" type="text" />

		</div>
		<br>
		<div>
			<label>契約種類</label> <select id="契約種類" style="width: 150px"
				name="契約種類">
		...
		<div>
			<input type="submit" id="save_btn" Value="登録">
